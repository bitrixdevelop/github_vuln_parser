        ...
        for page in range(1, self.pages_total + 1):
            ...
            for advisory in advisories_arr:
                ...
                for sidebar in discussion_sidebar:
                    ...
                    if sidebar_title:
                        ...
                        if sidebar_title.text == 'CVE ID':
                            cve_id = sidebar.find('div', class_='color-fg-muted').text.replace('\n', '').strip()

                        if sidebar_title.text == 'GHSA ID':
                            ghsa_id = sidebar.find('div', class_='color-fg-muted').text.replace('\n', '').strip()
                ...
                if header_block:
                    ...
                    if header_subblocks:
                        package_name = header_subblocks[0].find('span',
                                                                class_=['f4', 'color-fg-default', 'text-bold']).text
                        affected_version_array = header_subblocks[1].find_all('div', class_=['f4', 'color-fg-default'])
                        # Преобразовываем массив в строку с разделителем ', '
                        affected_version = ', '.join(
                            [affected_version_item.text.replace('\n', '').strip() for affected_version_item in
                             affected_version_array])
                        patched_version_array = header_subblocks[2].find_all('div', class_=['f4', 'color-fg-default'])

                        patched_version = ', '.join(
                            [patched_version_item.text.replace('\n', '').strip() for patched_version_item in
                             patched_version_array])

                central_block = content_blocks[1]
                if central_block:
                    # Trying to find link a with attr data-hovercard-type="pull_request"
                    pull_request_links = central_block.find_all('a', attrs={'data-hovercard-type': 'pull_request'})
                    if pull_request_links:
                        pull_request_link = ', '.join(
                            [pull_request_link_item.get('href') for pull_request_link_item in
                             pull_request_links])
